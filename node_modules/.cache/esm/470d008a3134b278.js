let axios,chalk,FormData,fs,fetch,path,projectName,promisify,zipdir;_313‍.x([["syncPlayground",()=>syncPlayground]]);_313‍.w("axios",[["default",["axios"],function(v){axios=v}]]);_313‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);_313‍.w("form-data",[["default",["FormData"],function(v){FormData=v}]]);_313‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_313‍.w("node-fetch",[["default",["fetch"],function(v){fetch=v}]]);_313‍.w("path",[["default",["path"],function(v){path=v}]]);_313‍.w("project-name",[["default",["projectName"],function(v){projectName=v}]]);_313‍.w("util",[["promisify",["promisify"],function(v){promisify=v}]]);_313‍.w("zip-dir",[["default",["zipdir"],function(v){zipdir=v}]]);
...
//	console.log('sendZip', filepath, playgroundID);
	...
	try {
		...
		_313‍.g.console.log('%s Couldn\'t parse response! %s', chalk.red.bold('ERROR'), e);
	}
...
//  console.log('PLAYGROUND -->>', response);
	...
	try {
		...
		_313‍.g.console.log('%s Couldn\'t find build dir! %s', chalk.red.bold('ERROR'), e);
		process.exit(1);
	}

	const playgroundID = await getCache('playground_id');

	_313‍.g.console.log('%s Queueing playground…', chalk.cyan.bold('INFO'));
	let response = null;
	try {
		response = await queryPlayground(playgroundID);

	} catch (e) {
		_313‍.g.console.log('%s Error querying server! %s', chalk.red.bold('ERROR'), e);
		process.exit(1);
	}
	...
	_313‍.g.console.log('%s Compressing files…', chalk.cyan.bold('INFO'));
	await createZip(path.join(process.cwd(), 'build'), playground.id, async(zipPath)=> {
//		console.log('::::', 'ZIPPED');

		_313‍.g.console.log('%s Sending zip…', chalk.cyan.bold('INFO'));
		await sendZip(zipPath, playground.id);

		_313‍.g.console.log('%s Playground created! %s', chalk.green.bold('DONE'), chalk.blue.bold(`https://playground.designengine.ai/${playground.id}`));
	});
